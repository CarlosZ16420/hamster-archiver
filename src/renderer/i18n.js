...
const exactSections = [
  ...
  ['仓库·详情与整理信息', [
    ...
    ['该项目已在更新或压缩队列中，请先完成或取消该任务，再更新目录。', 'This item is already queued for refresh or compression. Complete or cancel that task before refreshing the folder.'],
    ['更新目录会重新核对本地目录与仓库清单；不会修改、移动或删除原文件。新增内容会直接合并，修改或删除内容会先显示变化并等待确认。', 'This compares the local folder with its Warehouse manifest. Source files are never changed, moved, or deleted. Additions merge automatically; modifications or deletions require review.'],
    ['这里只会更新仓库记录指向的原文件位置，不会立即重建清单。下次更新目录或压缩入库时会重新核对内容。', 'This only changes the stored source location. The manifest will be checked on the next refresh or compression.'],
    ['已开始检查目录变化', 'Folder check started'],
    ['原文件位置已更新', 'Original location updated'],
    ['部分“更新目录”任务仅在不压缩入库时有效，本次不会执行。', 'Some Refresh Folder tasks only apply to uncompressed intake and were not run.'],
  ...
  ['主进程·队列与核心模块（校验与错误）', [
    ...
    ['没有可压缩入库的普通任务；“更新目录”任务请使用不压缩入库继续。', 'No regular tasks are ready for compression. Continue Refresh Folder tasks with uncompressed intake.'],
  ...
  ['主进程·队列与核心模块（完成与状态提示）', [
    ...
    ['目录未发现变化；未重建预览，也未改写仓库内容。', 'No folder changes found; previews and Warehouse data were not rebuilt.'],
    ['已生成完整清单并直接入库（未压缩）', 'Manifest added without compression'],
    ['等待更新目录', 'Queued for folder refresh'],
    ...
    ['已确认变化，等待更新目录', 'Changes approved; folder refresh queued'],
...
const patternSections = [
  ...
  ['仓库操作与撤回', [
    ...
    [/^目录更新于：(.+)$/, 'Folder updated: $1'],
    [/^上次检查于：(.+)$/, 'Last checked: $1'],
