...
const exactSections = [
  ...
  ['主进程·存储与路径', [
    ...
    ['压缩包存储位置不存在；请先确认是否创建，或重新选择位置。', 'The archive folder is missing. Confirm creation or choose another folder first.'],
    ['压缩包存储位置不是文件夹。', 'The archive location isn’t a folder.'],
...
const patternSections = [
  ...
  ['批量与清理结果', [
    ...
    [/^有多个项目低于当前 (\d+) MB 的入库阈值。$/, 'Multiple items are below the current $1 MB minimum item size.'],
    [/^“(.+)”项目低于 (\d+) MB 的入库阈值，已跳过。$/, 'Skipped “$1”: below the $2 MB minimum item size.'],
