...
const exactSections = [
  ...
  ['通用按钮与短词', [
    ['调整列宽', 'Resize column'],
    ['拖动调整列宽；双击恢复默认布局', 'Drag to resize columns; double-click to reset the layout'],
    ['拖动调整列宽；双击恢复默认布局；末列边界固定', 'Drag to resize; double-click to reset; the last-column boundary stays fixed'],
    ['关闭', 'Close'],
    ['取消', 'Cancel'],
    ['确定', 'OK'],
    ['确认', 'Confirm'],
  ...
  ['工作台·更多设置', [
    ...
    ['数据与维护工具', 'Data & Tools'],
    ['保存设置', 'Save settings'],
    ['实验功能 · AI 助手接入', 'Experimental · AI assistant integration'],
    ['默认关闭', 'Off by default'],
    ['Hamster Archiver 不会把媒体文件上传到在线模型；启用后，AI 客户端会看到工具返回的路径、标题、标签、备注等元数据。每种接入可单独关闭。', 'Hamster Archiver does not upload media files to online models. Enabled AI clients can see returned paths, titles, tags, notes, and other metadata. Each integration can be disabled separately.'],
    ['Codex Skill 调用方式', 'Codex Skill invocation'],
    ['点名应用时允许自动触发', 'Allow when the app is named'],
    ['仅显式选择时使用', 'Explicit selection only'],
    ['短 Skill + 本机 CLI', 'Short Skill + local CLI'],
    ['可选的本机 stdio MCP', 'Optional local stdio MCP'],
    ['本机 MCP + Skill', 'Local MCP + Skill'],
    ['通用 MCP', 'Generic MCP'],
    ['生成可导入的本机配置', 'Generate an importable local config'],
    ['Windows ODR 自动发现', 'Windows ODR discovery'],
    ['安全预览；需要包身份，不降低系统保护', 'Safe preview; requires package identity without reducing protections'],
    ['未启用', 'Not enabled'],
    ['已启用', 'Enabled'],
    ['检测到用户修改', 'User changes detected'],
    ['需要修复', 'Repair needed'],
    ['请先检查', 'Review first'],
    ['当前不可安全启用', 'Not safely available'],
    ['可以启用预览', 'Preview available'],
    ['生成', 'Generate'],
    ['启用', 'Enable'],
    ['禁用', 'Disable'],
    ['检测', 'Detect'],
    ['预览', 'Preview'],
    ['修复', 'Repair'],
    ['接入已关闭；仓库和用户资料未更改', 'Integration disabled; Warehouse and user data unchanged'],
    ['实验接入已启用', 'Experimental integration enabled'],
    ['实验接入已修复', 'Experimental integration repaired']
  ]],
  ['队列·工具栏与汇总', [
    ['02 · 扫描与队列', '02 · QUEUE'],
    ...
    ['等待下次入库', 'Next Run'],
    ['重复项处理', 'Duplicates'],
    ['清除疑似重复项', 'Remove Suspected Duplicates'],
    ...
    ['目录检查 / SOURCE CHANGES', 'SOURCE CHANGES'],
    ['目录内容发生变化', 'Source Folder Changed'],
    ['正在读取对比报告…', 'Loading comparison…'],
    ['查看变化并决定', 'Review Changes'],
    ['本地内容', 'Local content'],
    ['变化明细', 'Change details'],
    ['旧清单信息不足，请确认是否采用本次完整扫描结果。', 'The old manifest is incomplete. Choose whether to use this full scan.'],
    ['新增', 'Added'],
    ['删除', 'Deleted'],
    ['新增目录', 'Folder added'],
    ['删除目录', 'Folder deleted'],
    ['跳过', 'Skip'],
    ['新建独立项目', 'Create Separate Item'],
    ['覆盖原项目', 'Replace Existing Item'],
    ['新建独立项目只新增仓库记录，不会复制原文件。', 'Creates a separate Warehouse record without copying source files.'],
  ...
  ['仓库·概览', [
    ...
    ['导入仓库', 'Import Warehouse'],
    ['并入外部仓库', 'Import Warehouse'],
    ['仓库页码候选', 'Warehouse page suggestions'],
    ['随机漫步', 'Surprise Me'],
    ['库存', 'Inventory'],
    ['标签', 'Tags'],
    ['查看库存记录统计', 'View Item Stats'],
    ['查看全部标签', 'View All Tags'],
  ...
  ['仓库·工具栏与批量操作', [
    ...
    ['按标签筛选', 'Filter by tag'],
    ['全部标签', 'All tags'],
    ['选择标签以筛选仓库内容', 'Choose a tag to filter Warehouse items'],
    ['按备份位置筛选', 'Filter by backup location'],
    ['全部备份位置', 'All backup locations'],
    ['按星级筛选', 'Filter by rating'],
    ['清除入库时间筛选', 'Clear inventory time filter'],
    ['全部星级', 'All ratings'],
    ['未评分', 'Unrated'],
    ['入库', 'Added'],
    ['入库时间', 'Inventory time'],
    ...
    ['取消全部选择', 'Deselect all items'],
    ['输入仓库页码', 'Enter Warehouse page number'],
    ['返回仓库工具栏', 'Back to library toolbar'],
    ['列表', 'List'],
    ['仓库工具', 'Warehouse tools'],
    ['刷新仓库', 'Refresh Warehouse'],
    ['设置仓库位置', 'Set Warehouse Location'],
    ['在文件浏览器中查看仓库', 'Open Warehouse in File Explorer'],
    ['不展示“未压缩”标签', 'Hide the “Uncompressed” tag'],
  ...
  ['仓库·详情与整理信息', [
    ...
    ['上次校对时间', 'Last reviewed'],
    ['正在撤回仓库操作', 'Undoing Warehouse changes'],
    ['重新设置原文件位置', 'Relocate Original'],
    ['开始检查', 'Start Check'],
    ['更新位置', 'Update Location'],
    ['检查原地址的文件与仓库是否一致，不会改动原文件。若有新增内容，则更新至仓库。若有修改与删除，则需要您手动确认才会更新至仓库。', 'Check whether the files at the original location match the Warehouse without changing source files. Additions update the Warehouse automatically. Modifications and deletions require your confirmation.'],
    ['逐项确认', 'Review Each Item'],
    ['缩略图视图 · 大 · 再次点击切换大小', 'Thumbnail View · Large · Click again to change size'],
    ['缩略图视图 · 中 · 再次点击切换大小', 'Thumbnail View · Medium · Click again to change size'],
    ['缩略图视图 · 小 · 再次点击切换大小', 'Thumbnail View · Small · Click again to change size'],
    ['保留原位置', 'Keep Original Location'],
    ['确认备份位置', 'Confirm Backup Location'],
    ['备份位置不同，等待逐项确认', 'Backup location differs; awaiting individual confirmation'],
    ['已确认备份位置，等待库内项目压缩', 'Backup location confirmed; awaiting compression'],
    ['请先确认该项目的备份位置。', 'Confirm this item’s backup location first.'],
    ['该项目已在更新或压缩队列中，请先完成或取消该任务，再校对更新。', 'This item is already queued for refresh or compression. Complete or cancel that task before refreshing the folder.'],
    ['这里只会更新仓库记录指向的原文件位置，不会立即重建清单。下次校对更新或压缩入库时会重新核对内容。', 'This only changes the stored source location. The manifest will be checked on the next refresh or compression.'],
    ['已开始检查目录变化', 'Folder check started'],
    ['原文件位置已更新', 'Original location updated'],
    ['部分“校对更新”任务仅在不压缩入库时有效，本次不会执行。', 'Some Refresh Folder tasks only apply to uncompressed intake and were not run.'],
  ...
  ['风险确认与删除对话框', [
    ...
    ['不压缩入库会保存目录清单、预览和整理信息，原文件仍在原位置；校对更新不会修改、移动或删除原文件。本应用会跳过主目录中的零散图片和其他文件，如需处理，请先把它们收纳到文件夹中。', 'Uncompressed intake saves the manifest, previews, and organization details while originals stay in place. Refreshing a folder never changes, moves, or deletes source files. Loose files at the source root are skipped; put them in a folder first.'],
  ...
  ['渲染层·提示与确认', [
    ...
    ['项目名称已复制', 'Item name copied'],
    ['复制项目名称', 'Copy item name'],
    ...
    ['没有发现可清除的疑似重复任务', 'No suspected duplicate tasks to clear'],
    ...
    ['从任务列表清除所有名称或标题疑似重复、但不是完全重复的项目？已入库档案和源文件不会删除。', 'Remove suspected name/title duplicates, excluding exact duplicates? Archives and originals stay.'],
  ...
  ['主进程·队列与核心模块（校验与错误）', [
    ...
    ['没有可压缩入库的普通任务；“校对更新”任务请使用不压缩入库继续。', 'No regular tasks are ready for compression. Continue Refresh Folder tasks with uncompressed intake.'],
    ['当前任务没有等待确认的目录变化。', 'This task has no source changes awaiting review.'],
    ['查看同源项目', 'View items sharing this source'],
    ['不再提醒', 'Don’t show this notice again'],
    ['原自动封面已失效，请选择新的封面。', 'The original automatic cover is no longer valid. Choose a new cover.'],
    ['将按本地当前内容更新仓库目录及必要预览，不会修改原文件。', 'Update the Warehouse listing and necessary previews from the current source; originals will not change.'],
    ['将按本地当前内容压缩，并按现有设置在流程完成后处理原文件。', 'Compress the current source and handle originals afterward according to your existing settings.'],
    ['覆盖仓库记录，保留标签、备注等整理信息；不会把旧文件写回本地。', 'Replace the Warehouse listing while preserving tags and notes; old files will not be written back to the source.'],
    ['原仓库项目或原文件位置已变化，请取消任务后重新操作。', 'The Warehouse item or original location changed. Cancel this task and try again.'],
    ['压缩期间源文件或目录集合发生变化。', 'Source files or directories changed during compression.'],
    ['目录更新完成', 'Folder refresh completed'],
    ['目录更新已停止，等待继续原批次', 'Folder refresh stopped; continue its original batch'],
    ['已新建独立项目（未压缩）', 'Independent item added (uncompressed)'],
    ['仓库记录已提交，请核对尚未完成的原文件后处理；不会重复入库', 'The Warehouse record was committed. Check pending source handling; intake will not be repeated.'],
    ['仓库记录已提交，已恢复完成状态', 'The committed Warehouse record was restored as completed'],
    ['目录对比数据已经不可用，请取消任务后重新检查。', 'The comparison data is unavailable. Cancel the task and check again.'],
    ['请选择有效的目录变化处理方式。', 'Choose a valid source-change action.'],
    ['队列运行期间不能重新设置原文件位置。', 'Can’t relocate an original while the queue runs.'],
    ['只有未压缩仓库项目可以重新设置原文件位置。', 'Only uncompressed Warehouse items can relocate their originals.'],
    ['请先取消该项目尚未完成的更新或压缩任务。', 'Cancel this item’s pending refresh or compression task first.'],
    ['队列正在进行中，请结束后再进行批量更新。', 'Wait for the queue to finish before a batch refresh.'],
    ['所选内容中没有可更新的未压缩目录项目。', 'No selected uncompressed folders can be refreshed.'],
    ['原文件位置不可用或类型已经变化。请连接对应存储设备，或重新设置原文件位置。', 'The original location is unavailable or its type changed. Reconnect its storage or relocate the original.'],
  ...
  ['主进程·队列与核心模块（完成与状态提示）', [
    ...
    ['用户选择跳过目录变化；仓库记录和原文件均未修改。', 'Source changes skipped; the Warehouse item and originals were not changed.'],
    ['用户已确认按本地当前内容覆盖原仓库项目。', 'Approved replacing the existing Warehouse item with the current local content.'],
    ['用户已确认新建独立项目；原仓库项目保持不变。', 'Approved a separate item; the existing Warehouse item stays unchanged.'],
    ['已生成完整清单并直接入库（未压缩）', 'Manifest added without compression'],
    ['等待校对更新', 'Queued for folder refresh'],
    ['待核对原目录', 'Original folder pending check'],
    ['目录内容发生变化，等待确认', 'Source folder changed; awaiting review'],
    ['旧清单信息不足，等待确认当前目录', 'Old manifest incomplete; awaiting current-folder review'],
    ['目录未发现变化', 'No folder changes found'],
    ['已跳过目录变化，仓库与原文件均未修改', 'Source changes skipped; Warehouse item and originals unchanged'],
    ['已确认变化，等待校对更新', 'Changes approved; folder refresh queued'],
    ['已确认变化，等待覆盖并压缩', 'Changes approved; replacement compression queued'],
    ['已确认新建独立项目，等待不压缩入库', 'Separate item approved; uncompressed intake queued'],
    ['已确认新建独立项目，等待压缩', 'Separate item approved; compression queued'],
...
const runtimeLogSettingLabels = new Map([
  ['界面语言', 'interface language'],
  ['待处理目录', 'intake folder'],
  ['压缩包位置', 'archive location'],
  ['压缩暂存目录', 'archive staging folder'],
  ['归档后移动位置', 'post-archive move location'],
  ['归档后移动原文件', 'move originals after archiving'],
  ['归档后移入回收站', 'move originals to Recycle Bin after archiving'],
  ['记录备份位置', 'record backup location'],
  ['默认备份位置', 'default backup location'],
  ['视频抽帧', 'video frame previews'],
  ['视频抽帧数量', 'video frame count'],
  ['缩略图上限', 'thumbnail limit'],
  ['压缩格式', 'archive format'],
  ['压缩等级', 'compression level'],
  ['分卷压缩', 'split archives'],
  ['单卷大小', 'volume size'],
  ['压缩包命名方式', 'archive naming'],
  ['自定义压缩包名称', 'custom archive name'],
  ['压缩密码（内容未记录）', 'archive password (value not logged)'],
  ['记录压缩密码', 'record archive password'],
  ['小项目过滤', 'small-item filter'],
  ['小项目阈值', 'small-item threshold'],
  ['自动跳过完全重复项目', 'automatically skip exact duplicates'],
  ['自动跳过后的队列处理', 'queue action after automatic skip'],
  ['队列相似报告', 'queue similarity report'],
  ['相似度计算', 'similarity detection'],
  ['相似度强度', 'similarity strength'],
  ['超大文件夹简化处理', 'large-folder performance mode'],
  ['超大文件夹阈值', 'large-folder threshold'],
  ['代表文件数量', 'representative file count'],
  ['极小文件 MD5 跳过', 'tiny-file MD5 skipping'],
  ['极小文件阈值', 'tiny-file threshold'],
  ['定时运行', 'scheduled runs'],
  ['定时开始时间', 'schedule start'],
  ['定时结束时间', 'schedule end'],
  ['新手引导提示', 'onboarding prompts'],
  ['不压缩入库风险提示', 'inventory-only risk prompt'],
  ['库内压缩风险提示', 'Warehouse compression risk prompt']
]);
const runtimeLogMigrationModes = new Map([
  ['复制当前数据', 'copied current data'],
  ['切换到已有数据', 'switched to existing data'],
  ['保持当前位置', 'kept current location']
]);

function translateRuntimeLogSettingLabels(value) {
  return String(value || '')
    .split('、')
    .map((label) => runtimeLogSettingLabels.get(label) || label)
    .join(', ');
}

function translateRuntimeLogMigrationMode(value) {
  return runtimeLogMigrationModes.get(value) || value;
}

const patternSections = [
  ['运行日志与维护', [
    [/^应用已启动：版本 (.+)。$/, 'Application started: version $1.'],
    [/^应用正在正常退出。$/, 'Application is exiting normally.'],
    [/^无法读取历史运行日志：(.+)$/, 'Couldn’t read previous runtime logs: $1'],
    [/^运行日志未能写入文件：(.+)$/, 'Runtime log couldn’t be written to file: $1'],
    [/^设置已保存；变更：(.+)。$/, ([labels]) => `Settings saved; changed: ${translateRuntimeLogSettingLabels(labels)}.`],
    [/^已加入相似度排除词“(.+)”；已有关系不会自动重算。$/, 'Added “$1” to the similarity ignore list; existing links were not recalculated.'],
    [/^删除仓库内容“(.+)”失败：(.+)$/, 'Couldn’t delete Warehouse item “$1”: $2', { translateCaptures: [2] }],
    [/^删除仓库内容失败：没有找到记录 (.+)。$/, 'Couldn’t delete Warehouse item: record $1 was not found.'],
    [/^开始切换用户数据区：(.+) → (.+)。$/, 'Switching user data area: $1 → $2.'],
    [/^用户数据区切换完成：(.+) → (.+)；方式：(.+)。$/, ([from, to, mode]) => `User data area switched: ${from} → ${to}; mode: ${translateRuntimeLogMigrationMode(mode)}.`],
    [/^用户数据区已切换，但新位置的运行日志写入失败：(.+)$/, 'User data area switched, but the runtime log couldn’t be written at the new location: $1'],
    [/^切换用户数据区失败：(.+)$/, 'User data area switch failed: $1'],
    [/^检查更新完成：发现版本 (.+)。$/, 'Update check complete: version $1 is available.'],
    [/^检查更新完成：当前已是最新版本 (.+)。$/, 'Update check complete: $1 is current.'],
    [/^更新包 (.+) 已校验；用户选择稍后重启。$/, 'Update package $1 verified; restart postponed.'],
    [/^更新包 (.+) 已校验，应用将重启并执行更新。$/, 'Update package $1 verified; restarting to update.'],
    [/^安装程序 (.+) 已校验；用户选择稍后安装。$/, 'Installer $1 verified; installation postponed.'],
    [/^安装程序 (.+) 已校验，应用将退出并启动安装。$/, 'Installer $1 verified; exiting to start installation.'],
    [/^启动更新助手失败：(.+)$/, 'Couldn’t start the update helper: $1'],
    [/^启动安装程序失败：(.+)$/, 'Couldn’t start the installer: $1'],
    [/^准备版本 (.+) 更新失败：(.+)$/, 'Couldn’t prepare update $1: $2'],
    [/^准备本地更新包失败：(.+)$/, 'Couldn’t prepare the local update package: $1'],
    [/^界面预加载失败：(.+)$/, 'Interface preload failed: $1'],
    [/^定时运行检查失败：(.+)$/, 'Scheduled-run check failed: $1'],
    [/^AI 任务处理失败：(.+)$/, 'AI task processing failed: $1'],
    [/^应用启动失败：(.+)$/, 'Application startup failed: $1'],
    [/^切换仓库位置失败：(.+)$/, 'Warehouse location switch failed: $1'],
    [/^导出仓库失败：(.+)$/, 'Warehouse export failed: $1'],
    [/^并入外部仓库失败：(.+)$/, 'Warehouse import failed: $1'],
    [/^检查更新失败：(.+)$/, 'Update check failed: $1'],
    [/^重新载入相似度排除词失败：(.+)$/, 'Similarity ignore-list reload failed: $1'],
    [/^新增相似度排除词失败：(.+)$/, 'Adding a similarity ignore term failed: $1'],
    [/^全局重算相似关系失败：(.+)$/, 'Full similarity rebuild failed: $1'],
    [/^保存设置失败：(.+)$/, 'Saving settings failed: $1'],
    [/^扫描待处理目录失败：(.+)$/, 'Intake folder scan failed: $1'],
    [/^复原原文件失败：(.+)$/, 'Original-file restoration failed: $1'],
    [/^撤回仓库操作失败：(.+)$/, 'Warehouse undo failed: $1']
  ]],
  ['仓库后台加载', [
    [/^正在后台加载仓库…$/, 'Loading the warehouse in the background…'],
    [/^正在后台加载仓库（(\d+)\/(\d+)）…$/, 'Loading the warehouse in the background ($1/$2)…'],
    [/^仓库加载失败：(.+)$/, 'Warehouse loading failed: $1']
  ]],
  ['相似度·重算进度', [
  ...
  ['队列与任务', [
    ...
    [/^其中 (\d+) 项不在当前页$/, ([count]) => `${countNoun(count, 'item')} on other pages`],
    ...
    [/^入库 (.+)$/, 'Added $1'],
    [/ · 入库 /, '· Added'],
    [/^入库时间：(.+) · 清除$/, 'Inventory time: $1 · Clear'],
    [/^入库时间：/, 'Inventory time:'],
    [/^原始名称：/, 'Original name:'],
  ...
  ['仓库与分页', [
    [/^(.+)与本地目录内容一致$/, '$1 matches the local folder contents'],
    [/^项目“(.+)”与本地目录内容一致；未重建预览，也未改写仓库内容。$/, '“$1” matches the local folder contents; previews and Warehouse contents were not rebuilt.'],
    [/^项目“(.+)”已确认备份位置：(.+)。$/, '“$1” backup location confirmed: $2.'],
    [/^原项目记录的备份位置为：(.+)，当前压缩设置的备份位置为：(.+)，备份位置是否更新？$/, 'The item’s original backup location is: $1. The current compression backup location is: $2. Update the backup location?'],
    [/^所选项目有(\d+)项的备份位置与当前压缩设置不同，当前压缩设置为“备份位置：(.+)”，是否将项目备份位置更新为当前设置？$/, '$1 selected items have a different backup location. The current compression setting is “Backup location: $2”. Update the items’ backup locations to this setting?'],
    [/^\/ (\d+) 页 · 共 (\d+) 项$/, ([pages, count]) => `/ ${pages} · ${countNoun(count, 'item')} total`],
  ...
  ['批量与清理结果', [
    ...
    [/^已清除 (\d+) 个疑似重复任务$/, ([count]) => `Cleared ${countNoun(count, 'suspected duplicate task')}`],
    ...
    [/^(\d+) 个项目未能更新；(\d+) 个已开始检查$/, ([failed, started]) => `${countNoun(failed, 'item')} couldn’t be refreshed; ${countNoun(started, 'item')} started`],
    [/^(\d+) 个项目未能开始(?::|：)?(.*)；(\d+) 个已开始检查$/, ([failed, reason, started]) => `${countNoun(failed, 'item')} couldn’t start${reason ? `: ${reason}` : ''}; ${countNoun(started, 'item')} started`, { translateCaptures: [2] }],
    [/^已开始检查 (\d+) 个目录$/, ([count]) => `Started checking ${countNoun(count, 'folder')}`],
    [/^新增 (\d+) 个文件 · 修改 (\d+) 个文件 · 删除 (\d+) 个文件$/, ([added, modified, deleted]) => `${added} added · ${modified} modified · ${deleted} deleted`],
    [/^新增目录 (\d+) 个 · 删除目录 (\d+) 个 · 未变化 (\d+) 个文件$/, ([added, deleted, unchanged]) => `${added} folders added · ${deleted} folders deleted · ${unchanged} files unchanged`],
    [/^另有 (\d+) 个仓库项目使用相同的原文件位置。$/, ([count]) => `${countNoun(count, 'other Warehouse item')} ${Number(count) === 1 ? 'uses' : 'use'} the same original location.`],
    [/^其余 (\d+) 项未在界面展开。$/, ([count]) => `${countNoun(count, 'additional change')} not shown.`],
    [/^将检查 (\d+) 个目录与仓库清单的差异；另有 (\d+) 项不是可更新的未压缩目录，将自动排除。更新不会修改、移动或删除原文件。$/, ([checked, excluded]) => `Compare ${countNoun(checked, 'folder')} with the Warehouse manifest; exclude ${countNoun(excluded, 'item')} that can’t be refreshed. Source files won’t be changed, moved, or deleted.`],
    [/^将检查 (\d+) 个目录与仓库清单的差异。更新不会修改、移动或删除原文件。$/, ([checked]) => `Compare ${countNoun(checked, 'folder')} with the Warehouse manifest. Source files won’t be changed, moved, or deleted.`],
    ...
    [/^已建立目录更新批次，共 (\d+) 项；只执行本批任务。$/, ([count]) => `Created a ${countNoun(count, 'folder-refresh task')} batch; only this batch will run.`],
    [/^目录对比等待确认：新增 (\d+)、修改 (\d+)、删除 (\d+) 个文件。$/, ([added, modified, deleted]) => `Source changes await review: ${added} added, ${modified} modified, ${deleted} deleted.`],
    [/^已重新设置“(.+)”的原文件位置；目录清单将在下次更新或压缩时核对。$/, 'Relocated the original for “$1”; its manifest will be checked on the next refresh or compression.'],
  ...
  ['仓库操作与撤回', [
    ...
    [/^修改 (\d+) 项备份位置$/, ([count]) => `Change ${countNoun(count, 'backup location')}`],
    [/^界面提示：(.+)$/, 'Toast: $1', { translateCaptures: [1] }],
  ...
  ['扫描与文件校验', [
    ...
    [/^生成预览期间源文件消失：(.+)$/, 'Source disappeared during preview generation: $1'],
    [/^生成预览期间源文件发生变化：(.+)$/, 'Source changed during preview generation: $1'],
