...
const exactSections = [
  ...
  ['工作台·收存位置', [
    ...
    ['扫描时会把所选目录下的每个文件夹或视频分别加入队列，跳过其他根级文件；启用小项目过滤时，低于当前阈值的项目也不会入队（默认 100 MB）', 'Scanning queues each folder or video directly under the selected directory and skips other root-level files. When small-item filtering is enabled, items below the current threshold are also excluded (100 MB by default)'],
    ['选填', 'Optional'],
    ['必填', 'Required'],
  ...
  ['队列·工具栏与汇总', [
    ...
    ['确认内容一致并继续', 'Confirm identical content and continue'],
    ['确认相似并继续', 'Confirm similarity and continue'],
    ...
    ['的文件夹，', 'files,'],
    ['选取', 'select'],
    ['个代表文件记录 MD5、计算相似度', 'representative files for MD5 recording and similarity analysis'],
    ...
    ['代表文件数量必须是 1—100000 的整数。', 'The representative file count must be an integer from 1 to 100000.'],
    ...
    ['同意全部重复项', 'Confirm all duplicate items'],
    ['清理队列', 'Clean queue'],
    ...
    ['内容完全一致的文件会在队列生成 MD5 后显示；当前报告只显示名称和大小证据。', 'Identical content appears after the queue generates MD5 values; this report currently shows name and size evidence only.'],
    ['其余内容完全一致的文件会在队列生成 MD5 后显示；当前已复用仓库中同一源项目未变化文件的已有 MD5。', 'Identical content for the remaining files appears after the queue generates MD5 values; existing MD5 values were reused for unchanged files from the same warehouse source.'],
  ...
  ['使用说明对话框', [
    ...
    ['扫描只把每个大文件夹与视频加入队列；随后由您选择压缩入库或不压缩直接入库。', 'Scanning only adds each large folder and video to the queue; you then choose compressed or uncompressed intake.'],
  ...
  ['渲染层·提示与确认', [
    ...
    ['从任务列表清除所有“名称可能重复”或“内容完全一致”的项目？已入库档案和源文件不会删除。', 'Clear all tasks marked as possible name duplicates or identical content? Archived files and source files will not be deleted.'],
    ...
    ['从任务列表清除所有完全重复项（项目完全重复或含内容完全一致的文件）？已入库档案和源文件不会删除。', 'Clear all complete duplicate items (complete project duplicates or items containing identical-content files)? Archived files and source files will not be deleted.'],
  ...
  ['主进程·队列与核心模块（校验与错误）', [
    ...
    ['要加入白名单的词语不能包含换行或控制字符。', 'A whitelist term cannot contain line breaks or control characters.'],
    ['要加入白名单的词语不能为空。', 'A whitelist term cannot be empty.'],
    ['要加入白名单的词语不能超过 200 个字符。', 'A whitelist term cannot exceed 200 characters.'],
    ['要加入白名单的词语至少包含一个文字或数字。', 'A whitelist term must contain at least one letter or number.'],
    ...
    ['所选目录已经不存在。', 'The selected directory no longer exists.'],
    ...
    ['等待内容完全一致核验', 'Waiting for identical-content verification'],
    ...
    ['用户已确认内容完全一致提示，任务重新进入队列。', 'The user confirmed the identical-content warning and returned the task to the queue.'],
    ['用户已确认内容完全一致提示，任务复用已生成清单并重新进入队列。', 'The user confirmed the identical-content warning; the task reused its generated manifest and returned to the queue.'],
    ['用户已确认相似报告，任务复用已生成清单并重新进入队列。', 'The user confirmed the similarity report; the task reused its generated manifest and returned to the queue.'],
    ['用户已确认名称或相似项目提示；内容完全一致核验仍将在生成完整 MD5 后执行。', 'The user confirmed the name or similarity warning; identical-content verification will still run after the complete MD5 manifest is generated.'],
    ['名称重复', 'Duplicate name'],
  ...
  ['主进程·更新与媒体', [
    ['发行源返回了无法识别的版本号。', 'The release provider returned an unrecognized version number.'],
    ...
    ['SHA256 摘要地址不是受信任的 CNB HTTPS 地址。', 'The SHA256 digest URL is not a trusted CNB HTTPS address.'],
    ['更新包地址不是受信任的 GitHub HTTPS 地址。', 'The update package URL is not a trusted GitHub HTTPS address.'],
    ['更新包地址不是受信任的 CNB HTTPS 地址。', 'The update package URL is not a trusted CNB HTTPS address.'],
    ['CNB 更新元数据缺少受信任的来源标记。', 'CNB update metadata is missing its trusted source marker.'],
    ['CNB 更新元数据与本机信任配置不一致。', 'CNB update metadata does not match the local trust configuration.'],
    ['未配置 CNB Release API、历史 API 和发布页，已安全跳过 CNB 回退。', 'CNB Release APIs and release page are not configured, so CNB fallback was safely skipped.'],
    ['CNB 回退配置不完整；必须同时配置最新 Release API、历史 API 和发布页。', 'CNB fallback configuration is incomplete; latest/history APIs and the release page are all required.'],
...
const patternSections = [
  ...
  ['批量与清理结果', [
    ...
    [/^已清除 (\d+) 个完全重复任务$/, 'Cleared $1 complete duplicate tasks'],
    ...
    [/^“(.+)”已加入相似度白名单；已有关系不会自动重算$/, '“$1” added to the similarity whitelist; existing relations were not recalculated'],
    [/^“(.+)”已在相似度白名单中$/, '“$1” is already in the similarity whitelist'],
    ...
    [/^已选择压缩入库，共 (\d+) 个任务。$/, 'Compressed intake selected for $1 tasks.'],
    [/^该项目只有 ([\d.]+) MB，低于当前 (\d+) MB 的入库阈值。$/, 'This item is only $1 MB, below the current $2 MB intake threshold.'],
    [/^已添加单项任务：(.+)$/, 'Added single task: $1'],
    [/^未能复用已有清单进行即时内容完全一致核验，将在队列运行时重新计算 MD5：(.+)$/, 'Could not reuse the existing manifest for immediate identical-content verification; MD5 will be recalculated when the queue runs: $1']
  ]],
  ['仓库操作与撤回', [
    [/^撤回：(.+)$/, 'Undo: $1'],
    [/^已撤回：(.+)。$/, 'Undone: $1.'],
  ...
  ['扫描与文件校验', [
    ...
    [/^(\d+) 个低于 ([\d.]+) MB 的小项目$/, '$1 small items below $2 MB'],
    [/^(\d+) 个根目录非视频文件$/, '$1 root-level non-video files'],
    [/^(\d+) 个链接或重解析点$/, '$1 links or reparse points'],
    [/^(\d+) 个无法读取的项目$/, '$1 unreadable items'],
    [/^(\d+) 个不支持的项目$/, '$1 unsupported items'],
  ...
  ['更新与网络', [
    ...
    [/^检查更新失败：(.+)；(.+)$/, 'Update check failed: $1; $2'],
    [/^检查更新失败：(.*)$/, 'Update check failed: $1'],
    [/^(GitHub|CNB) 请求超时$/, '$1 request timed out'],
    [/^(GitHub|CNB) 连接失败：(.+)$/, '$1 connection failed: $2'],
    [/^(GitHub|CNB) 更新检查失败（HTTP (\d+)）$/, '$1 update check failed (HTTP $2)'],
    [/^(GitHub|CNB) Release 响应解析失败：(.+)$/, '$1 Release response could not be parsed: $2'],
    [/^(GitHub|CNB) Release 响应缺少有效的正式版本$/, '$1 Release response has no valid stable version'],
    [/^CNB 回退配置无效：(.+)$/, 'CNB fallback configuration is invalid: $1'],
    [/^不支持的更新来源：(.+)。$/, 'Unsupported update provider: $1.'],
    [/^(.+)重定向地址不是受信任的 (GitHub|CNB) HTTPS 地址。$/, 'The $1 redirect URL is not a trusted $2 HTTPS address.'],
  ...
  ['队列阶段与重复提示', [
    ...
    [/^发现 (\d+) 个相似候选$/, 'Found $1 similar candidates'],
    [/^完整项目与仓库内容完全一致$/, 'The complete project is identical to warehouse content'],
    [/^项目完全重复$/, 'Complete project duplicate'],
    ...
    [/^(\d+) 个文件内容完全一致$/, '$1 file(s) with identical content'],
    [/^(\d+) 个目录名称完全一致$/, '$1 folder name(s) are identical'],
    [/^(\d+) 个文件名称相似$/, '$1 file name(s) are similar'],
    [/^(\d+) 个目录名称相似$/, '$1 folder name(s) are similar'],
    [/^内容完全一致候选待人工核对$/, 'Identical-content candidates require manual review'],
    [/^内容完全一致候选核验达到读取预算，未完成的候选已转为人工复核；不会自动跳过。$/, 'Identical-content candidate verification reached its read budget. Unfinished candidates require manual review and will not be auto-skipped.'],
    [/^内容完全一致候选已提前排除；读取 (\d+) 个文件后停止完整核验。$/, 'Identical-content candidates were eliminated early; full verification stopped after reading $1 files.'],
    [/^发现(.+)，需要确认后才能(.+)。$/, 'Found $1; confirmation is required before $2.'],
    [/^发现 (.+)，已延后等待确认$/, 'Found $1; deferred for confirmation'],
    [/^自动跳过项目完全重复的任务“(.+)”(：.+)?；源文件和仓库均未修改，队列项已删除。$/, 'Auto-skipped complete project duplicate “$1”$2; source files and warehouse unchanged; the queue item was removed.'],
    [/^自动跳过项目完全重复的任务“(.+)”(：.+)?；源文件和仓库均未修改，队列项已保留。$/, 'Auto-skipped complete project duplicate “$1”$2; source files and warehouse unchanged; the queue item was retained.'],
    ...
    [/^当前不在定时运行时段；已记录入库方式，队列将在计划开始时间自动运行。$/, 'Outside the scheduled window; the selected intake mode was recorded and the queue will start automatically at the scheduled time.'],
...
const stageFragmentSections = [
  ['队列阶段', [
    ...
    ['名称可能重复', 'Name may be duplicated'],
    ['名称存在仓库候选', 'A warehouse item has the same name'],
    ['等待内容完全一致核验', 'Waiting for identical-content verification'],
    ['内容完全一致候选待人工核对', 'Identical-content candidates require manual review'],
    ['已自动跳过', 'Auto-skipped'],
    ['与仓库中 ', 'Identical to '],
    ...
    ['等待不压缩入库', 'Queued for intake without compression'],
    ['等待选择入库方式', 'Choose an intake mode'],
    ['待选入库方式', 'Choose intake mode'],
    ['未压缩直接入库', 'Uncompressed intake'],
    ['库内项目压缩 · 等待压缩', 'Warehouse item compression · queued'],
    ['已确认，等待不压缩入库', 'Confirmed; queued for intake without compression'],
    ['已确认，等待库内项目压缩', 'Confirmed; queued for warehouse item compression'],
    ['已确认，等待压缩', 'Confirmed, queued for compression'],
    ['已确认，等待选择入库方式', 'Confirmed; choose an intake mode'],
    ['已批量确认重复风险，等待不压缩入库', 'Duplicate risk confirmed in bulk; queued for intake without compression'],
    ...
    ['已批量确认重复风险，等待选择入库方式', 'Duplicate risk confirmed in bulk; choose an intake mode'],
    ['队列已进入定时等待。', 'The queue is waiting for its scheduled window.'],
    ['已生成完整清单并直接入库（未压缩）', 'Manifest completed and added without compression'],
    ['异常成品已移入回收站，源项目保持原位', 'Abnormal archive moved to the Recycle Bin; source kept in place'],
