...
const exactSections = [
  ...
  ['队列·工具栏与汇总', [
    ...
    ['只有完整清单中每个文件都有有效 MD5，且文件数量、相对路径、大小和 MD5 全部一致时，才会判定为精确重复项目。名称或大小相同只会提示可能相似，不会报为精确重复。', 'A project is reported as an exact duplicate only when every file has a valid MD5 and the complete file count, relative paths, sizes and MD5 values all match. Matching names or sizes alone are only similarity hints.'],
  ...
  ['主进程·队列与核心模块（校验与错误）', [
    ...
    ['用户已确认精确重复提示，任务重新进入队列。', 'The user confirmed the exact-duplicate warning and returned the task to the queue.'],
    ['用户已确认精确重复提示，任务复用已生成清单并重新进入队列。', 'The user confirmed the exact-duplicate warning; the task reused its generated manifest and returned to the queue.'],
    ...
    ['用户已确认名称或相似项目提示；精确重复核验仍将在生成完整 MD5 后执行。', 'The user confirmed the name or similarity warning; exact-duplicate verification will still run after the complete MD5 manifest is generated.'],
    ['名称重复', 'Duplicate name'],
...
const patternSections = [
  ...
  ['队列阶段与重复提示', [
    ...
    [/^(\d+) 个完整项目精确重复$/, '$1 exact duplicate projects'],
    ...
    [/^精确重复候选待人工核对$/, 'Exact-duplicate candidates require manual review'],
    [/^精确重复候选核验达到读取预算，未完成的候选已转为人工复核；不会自动跳过。$/, 'Exact-duplicate candidate verification reached its read budget. Unfinished candidates require manual review and will not be auto-skipped.'],
    [/^精确重复候选已提前排除；读取 (\d+) 个文件后停止完整核验。$/, 'Exact-duplicate candidates were eliminated early; full verification stopped after reading $1 files.'],
...
const stageFragmentSections = [
  ['队列阶段', [
    ...
    ['正在筛选精确重复候选：', 'Filtering exact-duplicate candidates: '],
