...
async function upload(repo, tag) {
  ...
  const notes = path.join(root, 'docs', 'releases', `release-notes-${tag}.md`);
  const body = await fs.readFile(notes, 'utf8');
  if (!/^## 中文\s*$/m.test(body) || !/^## English\s*$/m.test(body)) throw new Error('Release notes require Chinese and English sections.');
  if (!release) {
...
module.exports = { completeDraft, parseArgs, planUploads, preflight, run, upload, validateTarget, verifyFiles };
