...
[![仓库概览、收藏封面与搜索筛选](README.assets/仓库-主目录.png)](README.assets/仓库-主目录.png)

[![仓库大缩略图视图](README.assets/大缩略图模式.png)](README.assets/大缩略图模式.png)
...
已经不压缩入库的目录可以在仓库详情中直接“校对更新”，也可以批量检查。应用在任务真正执行时只扫描一次：仅新增文件会自动合并；修改、删除或旧清单不完整时会先展示逐项变化，让你选择覆盖原项目、新建独立项目或跳过。未变化文件复用已有 MD5 和预览，不重复抽帧；整个过程不会修改、移动或删除原文件。再次拖入同一路径时，也会先引导到校对更新或压缩，而不是静默创建重复项目。
...
便携版完整目录为 `HamsterArchiver-v4.6.18-win-x64/`，请保留随附文件。

[![批量归档、进度追踪与重复确认](README.assets/归档工作台-主目录.png)](README.assets/归档工作台-主目录.png)
...
[![标签、星级、备份位置、视频抽帧与完整目录](README.assets/项目详情.png)](README.assets/项目详情.png)
...
- 大缩略图与文本列表满足不同浏览习惯；支持键盘翻页、批量追加标签、修改备份位置，以及支持操作的撤回；本次运行中误删的仓库条目及其 Windows 回收站文件也可直接撤回，相似关系会一并恢复。
...
- 列表与缩略图均支持框选、Ctrl 反选及 Shift 范围选择，可直接清空跨页选择。“仓库工具”可按偏好隐藏“未压缩”标签；向下浏览后可一键返回搜索与批量操作栏。
