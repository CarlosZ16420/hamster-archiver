...
- 安装版升级应运行更高版本的安装程序；应用内 ZIP 替换只用于便携版，避免安装器、卸载器和商店包状态失配。
...
打包后的 Windows x64 便携版从顶部“检查更新”进入统一更新窗口：启动时可静默发现新版，但应用不强制更新，也不会因后台检查主动弹窗；只有用户点击“检查更新”才显示当前版本、最新版本和更新内容。检测到新版时提供自动更新；无论是否有新版或联网是否成功，都可选择“手动更新”并打开本机发行 ZIP。两条更新路径都会检查版本、`release-manifest.json` 与关键文件完整性，拒绝同版、旧版、平台不符或被破坏的包；替换前显示目标版本内容，替换时使用同一套回滚与启动验证流程并始终排除 `userdata`，成功启动新版本后再显示一次本次变化。旧发行 ZIP 没有嵌入说明时允许继续安全更新，但明确标记为未附带更新说明。
