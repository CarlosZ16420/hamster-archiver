...
可选 MCP 入口由 `src/core/mcp-server.js` 在显式启用后监听本机随机端口；发行包根目录的薄 `.cmd` 使用 Electron 内置 Node 运行 `mcp-client.js`。发行包的紧凑 `ai-capabilities.json` 只声明 CLI、三个 MCP 工具与按需发现入口，具体能力由运行时发现，避免重复加载完整目录；当前客户端拒绝未知命令。Windows 桌面代理与直接回退都会清除派生进程的 Node 模式环境并关闭崩溃上报，但保留 Electron 的系统默认图形选择与 Chromium 渲染沙箱，避免固定 GPU 开关在多显示适配器环境中反向触发崩溃。已有普通实例通过 `second-instance` 启用 MCP，始终只有一个仓库写入者，该路径的启用失败也会写回本次短期诊断文件。
