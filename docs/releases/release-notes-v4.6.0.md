...
4.6.0 优化大型仓库浏览和归档入库流程，补充队列运行时的追加能力，并改善重复判断和 Windows 界面显示。
...
- 更新窗口按版本倒序展示本地版本之后的正式发行说明，并按当前语言显示中英文内容；移除标题旁装饰图标，版本序列改用绿色。
- 删除自动跳过设置中重复的长说明；完成状态和“相似报告”按钮在窄窗口保持单行显示。
- Windows 原生窗口改用包含多种原生尺寸帧的 ICO，改善标题栏小图标清晰度。

### 数据与安全说明

本版本不改变 SQLite 结构或用户数据位置。重复判断只有在同一原始位置且完整目录和文件元数据快照可确认时才跳过 MD5，并且仍受用户启用的自动跳过设置控制；无法确认时继续使用内容核验。归档成品移动仍执行身份复核，源文件后处理继续在验证和登记成功后进行。

## English

Hamster Archiver 4.6.0 improves large-warehouse browsing and archive intake, allows additions while a queue run is active, and refines duplicate handling and Windows UI rendering.
...
- The update window lists formal releases after the local version in descending order and shows the matching language. The decorative title icon was removed and version text is now green.
...
This release does not change the SQLite schema or user-data location. MD5 is skipped only when the same original source location and a complete directory and file-metadata snapshot are confirmed, and automatic skipping remains controlled by the user's setting; otherwise content verification continues. Archive moves still verify file identity, and source post-processing remains gated on successful verification and registration.
