...
- 当前用户数据区的 `logs/app.log`：跨应用重启保留的 JSON Lines 运行日志；界面启动时显示最近 300 条有效记录，文件本身不因关闭应用而清空。
...
应用内切换用户数据区时，开始记录写入旧位置；复制或切换完成后，结果、源位置、目标位置和采用方式写入新位置的 `logs/app.log`。空目标会连同现有日志一起复制，因此旧位置仍保留完整回退资料；目标已有数据时不合并两边日志。
